...
-(AFURLSessionManager *)manager2{
    if (!_manager2) {
        _manager2=[[AFURLSessionManager alloc] initWithSessionConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
    }
    return _manager2;
}
...
- (void)get:(NSString *)url params:(NSDictionary *)params success:(void (^)(id json))success failure:(void (^)(NSError *error))failure{
    ...
    if ([self isNetWorkConnectAvailiable]) {
        [self.manager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
            //YJLog(@"%lld   %lld   %f",downloadProgress.totalUnitCount,downloadProgress.completedUnitCount,downloadProgress.fractionCompleted);
...
//    [AFHTTPResponseSerializer serializer].acceptableContentTypes=[NSSet setWithObjects:@"application/json", @"text/html",@"text/json",@"text/javascript",@"text/plain", nil];
    
    NSURLSessionUploadTask *uploadTask;
    self.manager2.responseSerializer=[AFHTTPResponseSerializer serializer];//不解析
    uploadTask = [self.manager2
                  ...
                  completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                      if (error) {
                          failure(error);
                          //[MBProgressHUD showToast:@"貌似网络不太好！"];
                      } else {
...
//        [formData appendPartWithInputStream:stream name:@"pic" fileName:@"pic.jpg" length:len mimeType:@"image/jpeg"];
